...
void create_nodev(struct pagev *head,char value[20])
{
   struct pagev *node;
   node=(struct pagev *)malloc(sizeof(struct pagev));
   strcpy(node->pv,value);
   node->nextv=NULL;
   if(head==NULL)
   {
       head =node;
   ...
   {
       struct pagev *temp=head;
       node->nextv=temp;
       head=node;
   }
}
void create_nodek(struct kart *head,char value[20])
{
   struct kart *node;
...
void moveback()
{
    struct pagev *temp = headv;
    headv=headv->nextv;
    free(temp);
    char value[20];
    strcpy(value,headv->pv);
}
void showkart()
{
...
printf("\nNet amount : %f ",PRICE);
fprintf(fil,"Name of the product purchased by customer: ");
fprintf(fil,"%s",NAME);
fprintf(fil,"\nPrice of the product purchased by customer: %f",PRICE);
int bank;
